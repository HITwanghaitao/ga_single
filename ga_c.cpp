...
//void ga::SetParameters(double (*objf)(vector<double>, vector<double>), int nvars, vector<vector<double>> bounds)
{
	...
	{
		//x_low[j] = bounds[0][j]; //对应区间输入格式为2*nvars
		//x_high[j] = bounds[1][j];
		x_low[j] = bounds[j][0]; //对应区间输入格式为nvars*2
		x_high[j] = bounds[j][1];
	}
	/*
...
void ga::Optimization_iteration()
{
	...
	{
		result.x_fit[i] = x_best[i]; //存储得到的最优变量
	}
	result.y_best = 1 / best_fitness; //存储得到的最优值

	endTime = clock();//计时结束
	if (Is_Show_Result) //是否显示优化结果
	{
		cout << "****显示优化结果****" << endl;
		cout << "优化代数：" << E_gentic << "; " << "种群数：" << N_genetic << endl;
		cout << "最优变量:" << endl;
		for (int i = 0; i < N_variable; i++)
		{
...
void ga::setGeneration(int e_genetic) //设置进化代数
{
	E_gentic = e_genetic;
}
void ga::setPopulation(int n_genetic) //设置种群规模
{
	N_genetic = n_genetic;
}
void ga::set_seed(int seed) //设置随机数种子
{
	r.set_seed(seed);
}
